...
_Before using the code, download and unzip the candidate summary file for the 2016 campaign from this address: <https://www.kaggle.com/danerbland/electionfinance>.Do the same for the 2018 campaign through this link: <https://classic.fec.gov/data/CandidateSummary.do?format=html>. (Use the csv button.) Then move both files into your working directory._ 

_Note that the file descriptions are provided [here](https://classic.fec.gov/finance/disclosure/metadata/metadataforcandidatesummary.shtml) and [here](https://classic.fec.gov/data/CandidateSummary.do?format=html) respectively._

_This document is unfinished._

This document has two motivations:
\n1. Find out whether the Democrats really have received more funding than they did in 2016
\n2. Make a(n) (explainable) miniature model (based solely on contributions and incumbency status) to investigate the chances that the Democrats take the House

First, some plots to illustrate how Democrats' coffers are brimming.
```{r readin, include=FALSE}
#setwd("C:/Users/Luke/Dropbox/Data Journalism")
cont <- read.csv(file="CandidateSummaryAction1.csv", header=TRUE, sep=",")
cont18 <- read.csv(file="CandidateSummaryAction.csv", header=TRUE, sep=",")
...
ggplot(map.df, aes(x=long,y=lat,group=group))+
  ...
  geom_path()+ 
  coord_map()+
  labs(title = "Has the Money Changed Its Colour?")+
  scale_fill_gradient2(low = "blue", midpoint = 0, mid = "white", high = "red",
                       limits=c(min, max), breaks=seq(min, max,
                                                      by=(max-min)/5),
...
So it would seem that Democrats have persuaded contributors to favour them, but this inference is mistaken.

INSERT OTHER CHARTS



This prompts the second goal of this document: to investigate the Democrats' chances. They currently hold only 188 out of the total 435, well short of the 290 needed to secure the crucial supermajority.

The chart below exhibits the landscape of the current state of play.
...
g <- ggplot(map.df, aes(x=long,y=lat,group=group))+
  ...
  coord_map()+
  labs(title="The Current House Seats", subtitle="Excluding Alaska and Hawaii")+
